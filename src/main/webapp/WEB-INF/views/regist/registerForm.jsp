	...
	$(function(){
		...
		$("#idChk").click(function(){
			...
			if(!regId.test($("#id").val())){
				alert("아이디는 8 자리에서 12 자리까지 사용이 가능합니다.\n첫번째 글자는 영문자여야 하고, 영문자/숫자/_ 만 사용가능합니다.")
				return false;
			}
			
			var url = "<%=request.getContextPath()%>/idChk";
			var data = "id="+$("#id").val();
			$.ajax({
				url : url,
				data : data,
...
<div class="container">
	<form id="loginForm" method="post" action="<%=request.getContextPath()%>/loginOk">
		<div id = "label">
			<ul>
				<li>아이디</li>
				<li>비밀번호</li>
		...
		<div id="input">
			<ul>
				<li><input type="text" name="id" id="id" maxlength="12" size="12"/><input type="button" class="button" id="idChk" value="중복검사"/>
				...
				<li><input type="text" name="email1" id="email1" size="6"/>@<input type="text" id="email2" name="eamil2" size="6"/>
